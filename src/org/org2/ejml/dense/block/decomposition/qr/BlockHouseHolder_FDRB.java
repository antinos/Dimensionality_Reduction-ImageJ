...
package org.org2.ejml.dense.block.decomposition.qr;

import org.org2.ejml.data.FMatrixRBlock;
import org.org2.ejml.data.FSubmatrixD1;
import org.org2.ejml.dense.block.InnerMultiplication_FDRB;
import org.org2.ejml.dense.block.VectorOps_FDRB;
 ...
 * Assumptions:
 * <ul>
 *  <li> All submatrices are aligned along the inner blocks of the {@link FMatrixRBlock}.
...
public class BlockHouseHolder_FDRB {
     ...
     */
    public static boolean computeHouseHolderCol( final int blockLength, final FSubmatrixD1 Y,
                                                 final float[] gamma, final int i) {
     ...
     * @return If there was any problems or not. true = no problem.
     */
    public static boolean computeHouseHolderRow( final int blockLength, final FSubmatrixD1 Y,
                                                 final float[] gamma, final int i) {
    ...
    public static void rank1UpdateMultR_Col(final int blockLength ,
                                            final FSubmatrixD1 A , final int col , final float gamma )
    {
        final int width = Math.min(blockLength,A.col1 - A.col0);

        final float dataA[] = A.original.data;

        for( int j = col+1; j < width; j++ ) {

            // total = U^T * A(:,j)
            float total = innerProdCol(blockLength, A, col, width, j, width);
                                               ...
                                               final FSubmatrixD1 A , final int col , final float gamma )
    {
        ...
        for( int colStartJ = A.col0 + blockLength; colStartJ < A.col1; colStartJ += blockLength ) {
            ...
            for( int j = 0; j < widthJ; j++ ) {
                // total = U^T * A(:,j) * gamma
                float total = innerProdCol(blockLength, A, col, widthCol, (colStartJ-A.col0)+j, widthJ)*gamma;
    ...
    public static void rank1UpdateMultL_Row( final int blockLength ,
                                             ...
                                             final int row , final int colStart , final float gamma )
    {
        ...
        for( int i = row+1; i < height; i++ ) {
            // total = U^T * A(i,:)
            float total = innerProdRow(blockLength, A, row, A , i, zeroOffset );
    ...
    public static void rank1UpdateMultL_LeftCol( final int blockLength ,
                                                 final FSubmatrixD1 A ,
                                                 final int row , final float gamma , int zeroOffset )
    {
        final int heightU = Math.min(blockLength,A.row1 - A.row0);
        final int width = Math.min(blockLength,A.col1-A.col0);

        final float data[] = A.original.data;
        ...
        for( int blockStart = A.row0+blockLength; blockStart < A.row1; blockStart += blockLength) {
            ...
            for( int i = 0; i < heightA; i++ ) {
                ...
                // total = U^T * A(i,:)
                float total = innerProdRow(blockLength, A, row, A, i+(blockStart-A.row0), zeroOffset);
...
//                    for( int k = 0; k < height; k++ ) {
    ...
    public static void divideElementsCol(final int blockLength ,
                                         final FSubmatrixD1 Y , final int col , final float val ) {
        final int width = Math.min(blockLength,Y.col1-Y.col0);

        final float dataY[] = Y.original.data;
     ...
     *
     */
    public static float computeTauAndDivideCol( final int blockLength ,
                                                 final FSubmatrixD1 Y ,
                                                 final int col , final float max ) {
        final int width = Math.min(blockLength,Y.col1-Y.col0);
     ...
     *
     */
    public static float computeTauAndDivideRow( final int blockLength ,
                                                 final FSubmatrixD1 Y ,
                                                 final int row , int colStart , final float max ) {
     ...
     */
    public static float findMaxCol(final int blockLength , final FSubmatrixD1 Y , final int col )
    {
        final int width = Math.min(blockLength,Y.col1-Y.col0);
     ...
     * from zero is automatically taken in account based on the column.
     */
    public static float findMaxRow( final int blockLength ,
                                          final FSubmatrixD1 Y ,
    ...
    public static void computeW_Column(final int blockLength ,
                                       ...
                                       final float temp[], final float beta[] , int betaIndex ) {
    ...
    public static void initializeW(final int blockLength,
                                   final FSubmatrixD1 W, final FSubmatrixD1 Y,
                                   final int widthB, final float b) {

        final float dataW[] = W.original.data;
        final float dataY[] = Y.original.data;
     ...
     * column 'col'.
     */
    public static void computeZ(final int blockLength , final FSubmatrixD1 Y , final FSubmatrixD1 W,
                                final int col , final float []temp , final float beta )
    {
            ...
            if( i == Y.row0 ) {
                ...
                for( int k = 0; k < heightW; k++ , indexZ += width, indexW += width , indexV += width ) {
                    ...
                    if( k < col ) {  // zeros
                        ...
                        dataW[indexZ] = beta_neg*(1.0f + total);
                    } else { // normal data
...
//                for( int k = 0; k < heightW; k++ ,
     ...
     */
    public static void computeY_t_V( final int blockLength , final FSubmatrixD1 Y ,
                                     final int col , final float []temp )
    {
    ...
    public static void multAdd_zeros(final int blockLength ,
                                     final FSubmatrixD1 Y , final FSubmatrixD1 B ,
                                     final FSubmatrixD1 C )
    {
     ...
     * </p>
     */
    public static void multBlockAdd_zerosone( float[] dataA, float []dataB, float []dataC,
        ...
        for( int i = 0; i < heightA; i++ ) {
            for( int j = 0; j < widthC; j++ ) {
                float val = i < widthA ? dataB[i*widthC+j+indexB] : 0;
     ...
     * C = A^T * B
     */
    protected static void multTransABlockSet_lowerTriag( float[] dataA, float []dataB, float []dataC,
                                                         int indexA, int indexB, int indexC,
                                                         final int heightA, final int widthA, final int widthC) {
        for( int i = 0; i < widthA; i++ ) {
            for( int j = 0; j < widthC; j++ ) {
                float val = i < heightA ? dataB[i*widthC + j + indexB] : 0;
